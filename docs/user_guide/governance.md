...
|-----------|--------|---------|
| **Consumer Registry** | `spanforge.consumer` | Track which tools depend on which schema namespaces |
| **Event Governance** | `spanforge.governance` | Block or warn on specific event types via policy |
| **Deprecation Tracking** | `spanforge.deprecations` | Register and surface deprecation notices at runtime |
...
`spanforge` package cannot satisfy a registered consumer's minimum
...
```python
from spanforge.consumer import assert_compatible, IncompatibleSchemaError
...
```python
from spanforge.governance import set_global_policy
...
```python
from spanforge.deprecations import mark_deprecated
...
At import time, `spanforge.deprecations` pre-populates the global
...
import spanforge
from spanforge.consumer import register_consumer, assert_compatible
from spanforge.governance import EventGovernancePolicy, set_global_policy
...
# 4. Process events
from spanforge.governance import check_event
