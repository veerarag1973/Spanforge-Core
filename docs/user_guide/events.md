...
For full JSON Schema validation (requires `pip install "spanforge[jsonschema]"`):
...
For tools that use Pydantic v2 (requires `pip install "spanforge[pydantic]"`):
...
```python
from spanforge.ulid import generate, validate, extract_timestamp_ms
