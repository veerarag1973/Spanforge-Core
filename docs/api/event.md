...
| `unknown_fields` | `Dict[str, Any]` | *(read-only)* Unrecognised fields preserved from `from_dict()` round-trips. Empty dict if none. |
...
Unrecognised keys in *data* are preserved in `event.unknown_fields` and
included in `to_dict()` output, enabling lossless round-tripping of events
that contain extension or future-version fields.
