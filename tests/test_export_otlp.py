"""Tests for spanforge/export/otlp.py — OTLPExporter.
...
from spanforge.export.otlp import (
    OTLPExporter,
    ResourceAttributes,
    _compute_end_nano,
    _derive_span_id,
...
class TestOTLPExporterInit:
    ...
    def test_zero_timeout_raises(self) -> None:
        with pytest.raises(ValueError, match="timeout"):
            OTLPExporter("http://localhost", timeout=0.0, allow_private_addresses=True)  # NOSONAR

    def test_negative_timeout_raises(self) -> None:
        with pytest.raises(ValueError, match="timeout"):
            OTLPExporter("http://localhost", timeout=-1.0, allow_private_addresses=True)  # NOSONAR

    def test_zero_batch_size_raises(self) -> None:
        with pytest.raises(ValueError, match="batch_size"):
            OTLPExporter("http://localhost", batch_size=0, allow_private_addresses=True)  # NOSONAR

    def test_defaults_applied(self) -> None:
        exp = OTLPExporter("http://localhost", allow_private_addresses=True)  # NOSONAR
    ...
    def test_custom_headers_copied(self) -> None:
        ...
        exp = OTLPExporter("http://localhost", headers=headers, allow_private_addresses=True)  # NOSONAR
    ...
    def test_custom_resource_attrs(self) -> None:
        ...
        exp = OTLPExporter("http://localhost", resource_attrs=ra, allow_private_addresses=True)  # NOSONAR
        assert exp._resource_attrs is ra

    def test_default_resource_attrs_set(self) -> None:
        exp = OTLPExporter("http://localhost", allow_private_addresses=True)  # NOSONAR
        assert exp._resource_attrs.service_name == "spanforge"
...
class TestOTLPRepr:
    def test_repr_contains_endpoint(self) -> None:
        exp = OTLPExporter("http://localhost:4318", allow_private_addresses=True)  # NOSONAR
        assert "http://localhost:4318" in repr(exp)  # NOSONAR

    def test_repr_contains_batch_size(self) -> None:
        exp = OTLPExporter("http://localhost", batch_size=250, allow_private_addresses=True)  # NOSONAR
        assert "250" in repr(exp)

    def test_repr_does_not_leak_headers(self) -> None:
        exp = OTLPExporter("http://localhost", headers={"Authorization": "Bearer top-secret"}, allow_private_addresses=True)  # NOSONAR
