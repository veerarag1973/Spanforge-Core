"""Tests for spanforge.redact — PII redaction framework.
...
class TestPIINotRedactedError:
    ...
    def test_message_with_context(self) -> None:
        ...
        # Raw context is replaced by a hash (M11: prevents PII leakage in error messages).
        assert "export_to_otlp" not in str(err)
        assert "context-hash:" in str(err)
...
class TestAssertRedacted:
    ...
    def test_context_appears_in_error_message(self) -> None:
        ...
        # Raw context is replaced by a hash (M11: prevents PII leakage).
        assert "export_step" not in str(exc_info.value)
