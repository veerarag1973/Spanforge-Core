"""Tests for spanforge.integrations.openai and spanforge.integrations._pricing.
...
class TestNormalizeResponse:
    ...
    def test_null_usage_gives_zero_tokens(self) -> None:
        ...
        resp.model = "gpt-4o"
        resp.usage = None
        token_usage, _model_info, cost = normalize_response(resp)
...
class TestPatchLifecycle:
    def setup_method(self) -> None:
        # Remove real or leftover mock openai from sys.modules, then block the real
        # package so tests that expect ImportError work in envs where openai is installed.
        # Tests that need openai (e.g. test_patch_sets_flag) call _build_mock_openai()
        # which sets sys.modules["openai"] to a mock, overriding this sentinel.
        openai_keys = [k for k in sys.modules if k == "openai" or k.startswith("openai.")]
        for key in openai_keys:
            del sys.modules[key]
