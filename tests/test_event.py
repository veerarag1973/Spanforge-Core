"""Tests for spanforge.event — Event, Tags, and helpers.
...
# ===========================================================================
# Phase 2 — Event.payload immutability (MappingProxyType guard)
# ===========================================================================


@pytest.mark.unit
class TestEventPayloadImmutability:
    """Event.payload must be read-only after construction (Phase 2 — Bug Fix 2.1).

    The ``payload`` property returns a ``MappingProxyType``, so any attempt
    to write to it raises ``TypeError`` — even before ``sign()`` is called.
    """
    ...
    def test_payload_item_assignment_raises_type_error(
        ...
        with pytest.raises(TypeError):
            minimal_event.payload["injected"] = "bad"  # type: ignore[index]

    def test_payload_item_deletion_raises_type_error(
        self, minimal_event_kwargs: dict[str, Any]
    ) -> None:
        e = Event(**dict(minimal_event_kwargs, payload={"key": "value", "extra": 1}))
        with pytest.raises(TypeError):
            del e.payload["key"]  # type: ignore[attr-defined]

    def test_payload_update_raises_type_error(
        self, minimal_event: Event
    ) -> None:
        with pytest.raises((TypeError, AttributeError)):
            minimal_event.payload.update({"new": "value"})  # type: ignore[union-attr]

    def test_original_dict_mutation_does_not_affect_payload(
        self, minimal_event_kwargs: dict[str, Any]
    ) -> None:
        """Mutating the dict passed at construction must not mutate the stored payload."""
        original = {"status": "ok", "mutable": True}
        e = Event(**dict(minimal_event_kwargs, payload=original))
        # Mutate the original dict after construction
        original["injected"] = "bad"
        assert "injected" not in e.payload

    def test_payload_keys_are_accessible(
        self, minimal_event_kwargs: dict[str, Any]
    ) -> None:
        e = Event(**dict(minimal_event_kwargs, payload={"key": "value"}))
        assert e.payload["key"] == "value"

    def test_payload_is_consistent_across_reads(
        self, minimal_event: Event
    ) -> None:
        """Multiple reads of .payload must return the same logical content."""
        first = dict(minimal_event.payload)
        second = dict(minimal_event.payload)
        assert first == second
