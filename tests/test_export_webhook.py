"""Tests for spanforge/export/webhook.py — WebhookExporter.
...
from spanforge.export.webhook import (
    _SIGNATURE_HEADER,
    WebhookExporter,
    _sign_body,
)
...
# ---------------------------------------------------------------------------
# Phase 2 — SSRF private-IP blocking (_validate_http_url)
# ---------------------------------------------------------------------------


class TestSSRFProtection:
    """_validate_http_url must reject literal private/loopback IPs by default."""

    def test_rfc1918_192_168_blocked(self) -> None:
        with pytest.raises(ValueError, match="private"):
            WebhookExporter("http://192.168.1.100/hook")

    def test_rfc1918_10_blocked(self) -> None:
        with pytest.raises(ValueError, match="private"):
            WebhookExporter("http://10.0.0.1/hook")

    def test_rfc1918_172_16_blocked(self) -> None:
        with pytest.raises(ValueError, match="private"):
            WebhookExporter("http://172.16.0.1/hook")

    def test_loopback_127_blocked(self) -> None:
        with pytest.raises(ValueError, match="private"):
            WebhookExporter("http://127.0.0.1/hook")

    def test_ipv6_loopback_blocked(self) -> None:
        with pytest.raises(ValueError, match="private"):
            WebhookExporter("http://[::1]/hook")

    def test_allow_private_addresses_bypasses_block(self) -> None:
        # Must not raise when explicitly permitted
        exp = WebhookExporter(
    ...
    def test_public_ipv4_allowed(self) -> None:
        # 93.184.216.34 is example.com — not private
        exp = WebhookExporter("http://93.184.216.34/hook")
        assert exp is not None

    def test_hostname_allowed(self) -> None:
        # localhost is a private address; use allow_private_addresses=True in tests
        exp = WebhookExporter("http://localhost/hook", allow_private_addresses=True)
        assert exp is not None
    ...
    def test_zero_retries_fails_immediately(self) -> None:
        ...
        with patch("urllib.request.urlopen", side_effect=_fake), pytest.raises(ExportError):
            asyncio.run(exp.export(event))
