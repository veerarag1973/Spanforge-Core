"""Tests for spanforge._stream — emit_span, emit_agent_step, emit_agent_run, _build_source.
...
# ===========================================================================
# Phase 2 — on_export_error policy + get_export_error_count
# ===========================================================================


@pytest.mark.unit
class TestExportErrorPolicy:
    """_handle_export_error must respect the on_export_error config policy.

    Policies: 'warn' (default) | 'drop' | 'raise'
    """
    ...
    def test_policy_drop_is_silent(self) -> None:
        ...
        stream_mod._cached_exporter = broken
        span = Span(name="drop-span")
        span.end()
        # Must not raise and must not issue a warning
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            # Should NOT raise — drop policy is fully silent
            try:
                emit_span(span)
            except OSError:
    ...
    def test_get_export_error_count_increments(self) -> None:
        ...
        stream_mod._cached_exporter = broken
        before = get_export_error_count()
        span = Span(name="counter-span")
        span.end()
        emit_span(span)
        after = get_export_error_count()
