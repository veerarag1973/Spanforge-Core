"""Tests for spanforge.export.grafana (GrafanaLokiExporter)."""
...
class TestConstruction:
    ...
    def test_invalid_scheme_url_raises(self) -> None:
        with pytest.raises(ValueError, match="url"):
            GrafanaLokiExporter(url="ftp://invalid-scheme.example.com")  # NOSONAR
...
class TestHttpErrors:
    def test_http_error_raises_export_error(
        ...
        with patch("spanforge.export.grafana.urllib.request.urlopen", _fail):
            async def _run() -> None:
    ...
    def test_tenant_id_header_set(
        ...
        with patch("spanforge.export.grafana.urllib.request.urlopen", _mock_urlopen):
            async def _run() -> None:
